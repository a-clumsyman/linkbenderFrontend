...
export const URLFeed: React.FC = () => {
  ...
  const fetchFeed = async () => {
    try {
      const endpoint = selectedTag 
        ? `http://localhost:8000/search-by-tags?tags=${selectedTag}`
        : 'http://localhost:8000/scrapes';
  ...
  const fetchPopularTags = async () => {
    try {
      ...
      const data = await response.json();
      setPopularTags(data.tags.sort((a: PopularTag, b: PopularTag) => b.count - a.count).slice(0, 10));
    } catch (error) {
        ...
        {showFilters && (
          <div className="bg-gray-800 p-4 rounded-lg mb-6">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium mb-2">Minimum Grade</label>
                <select
                  value={gradeFilter || ''}
